...
const GPSPage_Db = ({ navigation }) => {
  const { projectCode, gpsEquipment } = useProject();
  ...
  const fetchChecklistTemplate = async () => {
    try {
      const res = await fetch("http://103.163.184.111:3000/gps_checklist");
      const data = await res.json();
      const matched = data.filter((item) =>
        gpsEquipment.includes(item.gps_name)
      );
      setGpsChecklistData(matched);
    } catch (error) {
      console.error("Error fetching GPS checklist:", error);
    }
  };
    ...
    try {
      ...
      const filtered = data.filter(
        (row) =>
          ...
          gpsEquipment.includes(row.gps_name)
      );
      if (filtered.length === 0) return;
      const latestByName = {};
      filtered.forEach((row) => {
        if (
          !latestByName[row.gps_name] ||
          row.id > latestByName[row.gps_name].id
        ) {
          latestByName[row.gps_name] = row;
        }
      });
      ...
      setGpsCheckedItems(initChecked);
      setGpsItemNotes(initNotes);
      setGpsNotesInput(overall);
    } catch (error) {
      console.error("Error fetching GPS database:", error);
    } finally {
      setLoading(false);
    }
  };

  const fetchGpsSecondCheckboxData = async () => {
    try {
      const response = await fetch(
        `http://103.163.184.111:3000/gps_database_in?project_code=${projectCode}`
      );
      ...
      filteredData.forEach((row) => {
        if (
          !latestRecords[row.gps_name] ||
          row.id > latestRecords[row.gps_name].id
        ) {
          latestRecords[row.gps_name] = row;
        }
      });
      ...
      gpsChecklistData.forEach((payload) => {
        const gpsName = payload.gps_name;
        const record = latestRecords[gpsName];

        if (record) {
          // Find all item fields in the record
          Object.keys(record).forEach((key) => {
            if (key.startsWith("item_")) {
              const mapKey = normalizeKey(`${gpsName}_${key}`);
  ...
  useFocusEffect(
    React.useCallback(() => {
      ...
      const loadData = async () => {
        ...
        await fetchGpsSecondCheckboxData();
  ...
  const toggleCheck = (key) => {
    setGpsCheckedItems((prev) => {
      const newState = { ...prev, [key]: !prev[key] };
      if (!isGpsEdited) setIsGpsEdited(true);
      return newState;
    });
  };

  const handleNoteChange = (key, text) => {
    setGpsItemNotes((prev) => {
  ...
  const toggleSecondCheck = async (mapKey) => {
    const newState = {
      ...gpsSecondCheckedItems,
      [mapKey]: !gpsSecondCheckedItems[mapKey],
    };

    setGpsSecondCheckedItems(newState);

    try {
      await AsyncStorage.setItem(
        `gpsSecondChecks_${projectCode}`,
  ...
  useFocusEffect(
      ...
      const loadData = async () => {
        try {
          ...
          if (savedChecks) {
            ...
            setGpsSecondCheckedItems(initialChecks);
          }
    ...
    }, [projectCode, isGpsEdited, gpsChecklistData.length])
      ...
      <ScrollView style={styles.container}>
        <View style={styles.header}>
          ...
          <Text style={styles.subText}>
            Equipment: {gpsEquipment.join(", ") || "N/A"}
          </Text>
        </View>
        <Text style={styles.notFoundText}>
          GPS for {gpsEquipment.join(", ") || "N/A"} Not Found
        </Text>
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      {gpsChecklistData.map((gps, idx) => (
        <View key={idx} style={styles.section}>
          <Text style={styles.sectionTitle}>{`${gps.gps_name}`}</Text>
          {Object.keys(gps)
            .filter((key) => key.startsWith("item_") && gps[key])
            .map((key) => {
              const label = gps[key];
              const mapKey = normalizeKey(`${gps.gps_name}_${key}`);
              const checked = gpsCheckedItems[mapKey];
              const note = gpsItemNotes[mapKey] || "";
              return (
                <View key={key} style={styles.itemContainer}>
                  ...
                  {buttonsDisabled ? (
                    <View
                      style={[
                        styles.checkButton,
                        gpsCheckedItems[mapKey] && styles.checkedButton,
                      ]}
                    >
                      {gpsCheckedItems[mapKey] && (
                  ...
                  {/* <TouchableOpacity
                    style={[
                      styles.checkButton,
                      { marginStart: 10 },
                      gpsSecondCheckedItems[mapKey] && styles.checkedButton,
                    ]}
                    onPress={() => toggleSecondCheck(mapKey)}
                  >
                    {gpsSecondCheckedItems[mapKey] && (
                      <AntDesign name="check" size={20} color="white" />
                    )}
                  </TouchableOpacity> */}
                  ...
                  {buttonsDisabled ? (
                    ...
                    <TouchableOpacity
                      style={[
                        styles.checkButton,
                        { marginStart: 10 },
                        gpsSecondCheckedItems[mapKey] && styles.checkedButtonIn,
                      ]}
                      onPress={() => toggleSecondCheck(mapKey)}
                    >
                      {gpsSecondCheckedItems[mapKey] && (
